...
public class Note {
    ...
    public static void l(String tag, String content) {
        ...
        logLargeMessage(String.format("Note %s", tag), logPageFormat, content, charsPerPage, totalPages, 1);
    }
    ...
    private static void logLargeMessage(String tag, String logPageFormat, String content, int charsPerPage, int totalPages, int page) {
        if (content.length() > charsPerPage) {
            android.util.Log.println(Log.DEBUG, tag, String.format(logPageFormat, page, totalPages, content.substring(0, charsPerPage)));
            logLargeMessage(tag, logPageFormat, content.substring(charsPerPage), charsPerPage, totalPages, page + 1);
        } else {
            android.util.Log.println(Log.DEBUG, tag, page <= 1 ? content : String.format(logPageFormat, page, totalPages, content));
        }
    }
    ...
    public static void setOnlineLogger(ILogOnline onlineLogger) {
        Note.onlineLogger = onlineLogger;
    }
